...
model = HierarchicalCNN(num_classes_hierarchy).to(device)  # No additional features passed
...
# File paths for logs and weights
log_directory = '/mnt/RAID/projects/FjordVision/models/ablations/remove_features/logs-coco/'
weights_directory = '/mnt/RAID/projects/FjordVision/models/ablations/remove_features/weights-coco/'
...
with open(log_filename, mode='w', newline='') as file:
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            targets = [batch[-4].to(device), batch[-3].to(device), batch[-2].to(device), batch[-1].to(device)]

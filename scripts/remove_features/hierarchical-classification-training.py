...
patience_counter = 0
criterion = HierarchicalCrossEntropyLoss(num_levels=len(num_classes_hierarchy), alpha=args.alpha, learnable_alpha=False, device=device)
optimizer = torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=0.01)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=10, verbose=True)

# Set up directories for logs and weights
log_directory = 'models/ablations/remove_features/logs'
weights_directory = 'models/ablations/remove_features/weights/'
